...
declare var mobileDebug: boolean;
declare var portraitDebug: boolean;
declare var isPlayer1: boolean;
declare var cratePositions: CratePosition[];
var player1: PlayerSprite;
var player2: PlayerSprite;
var clickable;
var map;
var layer1;
var layer2;
...
class BombSprite extends Phaser.Sprite {
    phyEnabledPlayer1: boolean;
    phyEnabledPlayer2: boolean;
    owner: PlayerSprite;
    flame: number;
}

class PowerUpSprite extends Phaser.Sprite {
    pickedUp: boolean;
}
...
function create() {
    ...
    if (!game.device.desktop || mobileDebug) {
        ...
        if (!portraitDebug || (<any>window).orientation == 90 || (<any>window).orientation == -90) {
            clickable.x = 720;
        } else {
            clickable.x = 0;
            clickable.y = 624;
    ...
    player1 = <PlayerSprite>game.add.sprite(48 + 24, 48 + 24, 'bomb_party_v4_bank_32s');
    player1.bombs = 1;
    ...
    // PLAYER 2 CREATIOn
    player2 = <PlayerSprite>game.add.sprite(48 * 13 + 24, 48 + 24, 'bomb_party_v4_bank_32s');
    player2.bombs = 1;
    ...
    if (!isPlayer1) {
        ...
        //player2.body.damping = 0;
        //player2.body.angularDamping = 0;
    }
    ...
    if (isPlayer1) controlKey.onUp.add(placeBomb, player1);
    else controlKey.onUp.add(placeBomb, player2);
    ...
    if (isPlayer1) {
        ...
        //        player1.body.collides([mapCollisionGroup]);
    } else {
...
function placeBomb() {
    ...
    if (isPlayer1) {
        if (p1wasUp) snapPosY += 24;
        if (p1wasDown) snapPosY -= 24;
        if (p1wasLeft) snapPosX += 20;
        if (p1wasRight) snapPosX -= 20;
    } else {
        if (p2wasUp) snapPosY += 24;
        if (p2wasDown) snapPosY -= 24;
        if (p2wasLeft) snapPosX += 20;
        if (p2wasRight) snapPosX -= 20;
    }
    ...
    if (bombs.filter(function (sprite) {
        ...
        var bomb = <BombSprite>game.add.sprite(posX, posY, 'bomb_party_v4_bank_32s');
...
function createExplosion(posX, posY, explosions, currentExplosionIndex, frame, explosionEndFrame) {
    ...
    if (anyCratesHit.length != 0) {
        ...
        game.time.events.add(Phaser.Timer.SECOND * 0.5, function () {
            ...
            if (crateToRemoveIndex > -1) crates.splice(crateToRemoveIndex, 1);
    ...
    if (currentExplosionIndex == 1) explosionRigth.frame = explosionEndFrame;
...
function detonateBomb(bomb, destroyExplosions) {
    if (bombs.indexOf(bomb) == -1) return;
    bomb.destroy();
    bomb.owner.bombsAvailable++;

    var bombToRemoveIndex = bombs.indexOf(bomb);
    if (bombToRemoveIndex > -1) bombs.splice(bombToRemoveIndex, 1);
    ...
    if (destroyExplosions) game.time.events.add(Phaser.Timer.SECOND * 0.5, removeExplosion, this, explosions);
    else return explosions;
}

function removeExplosion(explosions) {
    explosions.forEach(function (explosion) {
        var explosionToRemoveIndex = explosionsAll.indexOf(explosion);
        if (explosionToRemoveIndex > -1) explosionsAll.splice(explosionToRemoveIndex, 1);
        explosion.destroy();
    });
}
...
function endGame() {
    ...
    if (player1.isDead && player2.isDead) {
        ...
        if (isPlayer1) {
            if (player1.isDead) {
                var modal = <any>$('#myModalLost');
                text = "YOU LOST!";
                style.fill = "red";
                //                modal.modal('show');
            } else {
                var modal = <any>$('#myModalWon');
                text = "YOU WON!";
                style.fill = "lime";
                //                modal.modal('show');
            }
...
function destroyPlayer(player: PlayerSprite) {
    player.kill();
    player.destroy();
...
function update() {
    ...
    filtered.forEach(function (bomb) {
        if (player1.position.x + 24 < bomb.position.x - 24 || player1.position.x - 24 > bomb.position.x + 24
            || player1.position.y + 24 < bomb.position.y - 24 || player1.position.y - 24 > bomb.position.y + 24) {
    ...
    filtered.forEach(function (bomb) {
        if (player2.position.x + 24 < bomb.position.x - 24 || player2.position.x - 24 > bomb.position.x + 24
            || player2.position.y + 24 < bomb.position.y - 24 || player2.position.y - 24 > bomb.position.y + 24) {
    ...
    if (!game.device.desktop || mobileDebug) {
        if ((touchIsDown && inputMethod.x < 720 || game.input.pointer2.isDown && game.input.pointer2.x < 720) &&
            ((mobileDebug && !portraitDebug) || (<any>window).orientation == 90 || (<any>window).orientation == -90)) {
            placeBomb.apply(isPlayer1 ? player1 : player2);;
        } else if ((touchIsDown && inputMethod.y < 624 || game.input.pointer2.isDown && game.input.pointer2.y < 624) &&
            ((mobileDebug && portraitDebug) || (<any>window).orientation == 0)) {
            placeBomb.apply(isPlayer1 ? player1 : player2);
        }
    }
    ...
    if (isPlayer1) {
        if (cursors.left.isDown ||
            touchIsDown &&
            isLeftButtonTriggered()) {
            player1.body.setRectangle(26, 43, 2, 0);
            //            player1.body.setCircle(26);
            player1.animations.play('left', 4, true);
            if (player1.scale.x > 0) player1.scale.x *= -1;
            if (!p1wasLeft) {
                if (!cursors.up.isDown && !cursors.down.isDown) gameHub.server.playerTurn("left");
            }
            ...
            touchIsDown && isRightButtonTriggered()) {
            player1.body.setRectangle(26, 43, -2, 0);
            ...
            if (!p1wasRight) {
                if (!cursors.up.isDown && !cursors.down.isDown) gameHub.server.playerTurn("right");
            }
            p1wasDown = false;
            p1wasUp = false;
            p1wasLeft = false;
        ...
        if (cursors.up.isDown ||
            ...
            wasKeyDown = true;
            if (!p1wasUp) gameHub.server.playerTurn("up");
            p1wasDown = false;
            p1wasUp = false;
            p1wasLeft = false;
            p1wasRight = false;
            ...
            touchIsDown &&
            isDownButtonTriggered()) {
            player1.animations.play('down', 4, true);
            player1.body.setRectangle(40, 43, 0.5, 0.5);
            player1.body.moveDown(300);
            wasKeyDown = true;
            if (!p1wasDown) gameHub.server.playerTurn("down");
            p1wasUp = false;
            p1wasLeft = false;
            p1wasRight = false;
            p1wasDown = true;
        ...
        if (!wasKeyDown) {
            ...
            player1.animations.stop();
            if (p1wasDown || p1wasUp || p1wasLeft || p1wasRight) gameHub.server.stoppedMoving();
            if (p1wasDown) player1.frame = 17;
            if (p1wasUp) player1.frame = 16;
            if (p1wasLeft) player1.frame = 21;
    ...
    if (!isPlayer1) {
        ...
        if (cursors.left.isDown) {
            ...
            if (player2.scale.x > 0) player2.scale.x *= -1;

            if (!p2wasLeft) gameHub.server.playerTurn("left");
            p2wasRight = false;
            p2wasUp = false;
            p2wasDown = false;
            p2wasLeft = true;
            ...
            if (!p2wasRight) gameHub.server.playerTurn("right");
            p2wasLeft = false;
            p2wasUp = false;
            p2wasDown = false;
            p2wasRight = true;
        ...
        if (cursors.up.isDown) {
            ...
            wasKeyDown = true;
            if (!p2wasUp) gameHub.server.playerTurn("up");
            p2wasDown = false;
            p2wasLeft = false;
            p2wasRight = false;
            p2wasUp = true;
            ...
            wasKeyDown = true;
            if (!p2wasDown) gameHub.server.playerTurn("down");
            p2wasUp = false;
            p2wasRight = false;
            p2wasLeft = false;
            p2wasDown = true;
        ...
        if (!wasKeyDown) {
            ...
            player2.animations.stop();
            if (p2wasDown || p2wasUp || p2wasLeft || p2wasRight) gameHub.server.stoppedMoving();
            if (p2wasDown) player2.frame = 17 + 15;
            if (p2wasUp) player2.frame = 16 + 15;
            if (p2wasLeft) player2.frame = 21 + 15;
...
function isLeftButtonTriggered() {
    if (game.device.desktop && !mobileDebug) return false;
    ...
    if (!portraitDebug || (<any>window).orientation == 90 || (<any>window).orientation == -90) {
        return PointInTriangle({ x: inputMethod.x, y: inputMethod.y },
            { x: 720, y: 0 },
            { x: mywidth * 0.75, y: myheight / 2 },
            { x: 720, y: myheight }
            );
    } else {
        return PointInTriangle({ x: inputMethod.x, y: inputMethod.y },
            { x: 0, y: 624 },
            { x: mywidth / 2, y: myheight * 0.75 },
            { x: 0, y: myheight }
            );
    }
}
